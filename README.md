...
| 变量                  | 说明                   | 可选值 | 默认值 | 备注                     |
| --------------------- | ---------------------- | ------ | ------ | ------------------------ |
| DANDANPLAY_APP_ID     | 弹弹play API AppId     | 字符串 | 空     | 官方镜像已内置，无需配置 |
| DANDANPLAY_APP_SECRET | 弹弹play API AppSecret | 字符串 | 空     | 官方镜像已内置，无需配置 |

> **说明**：弹幕功能基于 [弹弹play开放平台](https://www.dandanplay.com/) 提供的弹幕库。
>
> - **使用官方 Docker 镜像**：弹幕功能开箱即用，无需任何配置。
> - **Fork 项目自行部署**：需要自行向弹弹play申请 API 凭证（发送邮件至 `kaedei@dandanplay.net`），然后在管理面板的「站点设置」中配置，或设置上述环境变量。
>
> 感谢 [弹弹play](https://www.dandanplay.com/) 为开源社区提供弹幕服务！
